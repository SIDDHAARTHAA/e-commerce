...
CREATE TABLE `users` (
    ...
    UNIQUE INDEX `users_defaultShippingAddressId_key`(`defaultShippingAddressId`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `orders` (
    ...
    `shippingLineOne` VARCHAR(191) NOT NULL,
    `shippingLineTwo` VARCHAR(191) NULL,
...
CREATE TABLE `order_events` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `orderId` INTEGER NOT NULL,
    `status` ENUM('PENDING', 'ACCEPTED', 'OUT_FOR_DELIVERY', 'DELIVERY', 'CANCELLED') NOT NULL DEFAULT 'PENDING',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
